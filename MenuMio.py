...
class MenuScreen(tk.Frame):
    def __init__(self, master):
        ...
        self.file_menu.add_command(label="Configuracion", command=self.setting_file)
        ...
        self.edit_menu.add_command(label="Remplazar", command=self.replace)
        self.menu_bar.add_cascade(label="Editar", menu=self.edit_menu)

        # Agregamos algunos widgets a la pantalla
        self.label = tk.Label(self, text="¡Hola, mundo!",bg="blue",fg="red")
        self.label.pack(pady=20)
        self.button = tk.Button(self, text="Presionar", command=self.press_button,bg="green")
        self.button.pack()
